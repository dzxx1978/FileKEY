...
## 参数：
  4. 参数`-v`，显示版本信息和全部可用参数列表,之后无论是否指定其他参数直接退出。  
     ~~~
        filekey.exe -v
     ~~~
  5. 参数`-0`，可以与第3种使用方法一同使用，只显示是否匹配后退出。  
     ~~~
  ...
  6. 参数`-t`，只显示文件类型。  
     ~~~
        filekey.exe -t
     ~~~
  7. 参数`-cms`，分别`-c`只显示crc，`-m`只显示md5，`-s`只显示sha256。可以单独使用，也可以任意组合使用，不分先后顺序。同时指定三个与不指定任何一个效果相同。  
     ~~~
        filekey.exe -cms
        filekey.exe -ms
        filekey.exe -s
